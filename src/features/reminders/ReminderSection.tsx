...
/** Herinnering voor gewone notities: tijd + eenmalig/dagelijks + snoozen. */
export function ReminderSection({ note }: Props) {
  ...
  return (
    <div className="shrink-0 space-y-2 border-t border-white/10 bg-[#181b23] px-3 py-2">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input
          type="datetime-local"
          className="rounded-md border border-white/10 bg-[#1a1d26] px-2 py-1 text-[11px] text-noto-text focus:outline-none focus:ring-1 focus:ring-emerald-500/40"
          value={localTime}
          onChange={(e) => setLocalTime(e.target.value)}
        />
        <select
          className="rounded-md border border-white/10 bg-[#1a1d26] px-2 py-1 text-[11px] text-noto-text focus:outline-none"
          value={repeat}
      ...
      {note.reminder ? (
        <div className="flex flex-wrap items-center gap-1.5">
          <span className="text-[10px] text-noto-muted">Uitstellen</span>
          {[5, 15, 60].map((m) => (
            <button
              key={m}
              type="button"
